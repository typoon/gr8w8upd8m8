...
def device_present(addr):
    """ This function will check query dbus about the status of the bluetooth 
    device that has the ID provided by the user. It returns True if the device
    is connected, False otherwise
    """
    try:
        pid = subprocess.check_output(["pidof", "bluetoothd"])
    except:
        pid = -1

    if int(pid) < 0:
        print "It seems like bluetoothd is down... FIX IT!"
        return False

    dev_id = addr.upper().replace(':', '_')
    pid = pid.replace("\n",'')
    dbus_path = "/org/bluez/%s/hci0/dev_%s" % (pid, dev_id)

    bus = dbus.SystemBus()
    dbus_obj = bus.get_object('org.bluez', dbus_path)

    result = dbus_obj.GetProperties(dbus_interface='org.bluez.Device')
...
def save_weight(weight):
    ...
    # determine whose weight we are trying to save right now.
    # Maybe in the future I will figure out something better than this
    person = 'Henrique'

    if weight < 70:
        person = 'Andreia'

    # If the weight is less than 40, it means that something is wrong...
    # Maybe it is just me doing some test
    if weight > 40:
        params = urllib.urlencode({'username': username, 'password': password, 'weight': weight, 'person': person})
        result = urllib.urlopen(remote_url, params)
        print result.read()

   
if __name__ == "__main__":
    # We need to monitor if the device has been paired and connected
    # If so, we execute the main function to read the weight and return back
    # to the loop checking for the connection
    address = sys.argv[1]
    try:
        # Disconnect already connected devices, otherwise we won't be able to
        # connect ourselves.
        subprocess.check_output(["bluez-test-device", "disconnect", address], stderr=subprocess.STDOUT)
        subprocess.check_output(["bluez-test-device", "disconnect", address], stderr=subprocess.STDOUT)
    except:
        pass
